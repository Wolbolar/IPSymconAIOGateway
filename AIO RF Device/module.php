...
class AIORFDevice extends IPSModule
{
    ...
    protected function CreateProfileRF($NumberRFCodes)
    {
        ...
        $rfprofilname = $this->clear_string($rfprofilname);
    ...
    protected function Send_RF($rf_code)
    {
        ...
        if ($GatewayPassword !== "")
        {
            ...
            {
                $gwcheck = file_get_contents("http://".$aiogatewayip."/command?auth=".$GatewayPassword."&code=".$rf_code."&XC_FNC=Send2&ir=00&rf=01");
                $this->SendDebug("RF Code",$rf_code,0);
                $this->SendDebug("AIOGateway","Senden an Gateway mit Passwort",0);
                $this->SendDebug("Send to AIO Gateway","http://".$aiogatewayip."/command?auth=".$GatewayPassword."&code=".$rf_code."&XC_FNC=Send2&ir=00&rf=01",0);
            }
            else
            {
        ...
        {
            $this->response = false;
            $this->SendDebug("AIOGateway","Keine Authentizifierung m�glich. Gateway Passwort ist falsch.",0);
        }
    ...
    protected function RegisterProfileIntegerRF($Name, $Icon, $Prefix, $Suffix, $MinValue, $MaxValue, $StepSize)
    {
    ...
    protected function RFProfileAssociation($Name, $MinValue, $MaxValue, $start)
    {
        ...
        for ($i = $MinValue; $i <= $MaxValue; $i++)
        {
            if (!empty($RFCodes[($start+$i)][1]))
            {
                ...
                IPS_SetVariableProfileAssociation( (string)$Name, (float)$i, (string)$RFCodes[($start+$i)][0], "", -1 );//max 32 m�glich
            }
    ...
    protected function RegisterProfileRFCodes($Name, $Icon, $start, $end, $NumberRFCodes)
    {
        ...
        $this->RegisterProfileIntegerRF($Name, $Icon, $Prefix, $Suffix, $MinValue, $MaxValue, 0);
        $this->RFProfileAssociation($Name, $MinValue, $MaxValue, $start);
    }
    ...
    public function Learn(int $irid)
    {
        ...
        if ($GatewayPassword !== "")
        {
            if($gatewaytype == 6 || $gatewaytype == 7)
            {
                $RFCode = file_get_contents("http://".$aiogatewayip."/command?auth=".$GatewayPassword."&XC_FNC=Learn");
            }
            else
            {
                $RFCode = file_get_contents("http://".$aiogatewayip."/command?XC_USER=user&XC_PASS=".$GatewayPassword."&XC_FNC=Learn");
            }
        }
        else
        {
            $RFCode = file_get_contents("http://".$aiogatewayip."/command?XC_FNC=Learn");
        }
