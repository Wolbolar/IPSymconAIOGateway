...
class AIOFS20Device extends IPSModule
{
    ...
    protected function SendCommand($command)
    {
        ...
        if ($GatewayPassword !== "")
        {
            ...
            {
                $gwcheck = file_get_contents("http://".$aiogatewayip."/command?auth=".$GatewayPassword."&XC_FNC=SendSC&type=FS20&data=".$FS20.$command);
                $this->SendDebug("String to AIO Gateway","http://".$aiogatewayip."/command?XC_USER=user&XC_PASS=".$GatewayPassword."&XC_FNC=SendSC&type=FS20&data=".$FS20.$command,0);
            }
            else
            {
                $gwcheck = file_get_contents("http://".$aiogatewayip."/command?XC_USER=user&XC_PASS=".$GatewayPassword."&XC_FNC=SendSC&type=FS20&data=".$FS20.$command);
                $this->SendDebug("String to AIO Gateway","http://".$aiogatewayip."/command?XC_USER=user&XC_PASS=".$GatewayPassword."&XC_FNC=SendSC&type=FS20&data=".$FS20.$command,0);
            }
        }
        else
        {
            $gwcheck = file_get_contents("http://".$aiogatewayip."/command?XC_FNC=SendSC&type=FS20&data=".$FS20.$command);
        }
        if ($gwcheck == "{XC_SUC}")
        {
            $this->response = true;
    ...
    public function Learn()
    {
        ...
        if ($GatewayPassword !== "")
        {
            if($gatewaytype == 6 || $gatewaytype == 7)
            {
                $address = file_get_contents("http://".$aiogatewayip."/command?auth=".$GatewayPassword."&XC_FNC=LearnSC&type=FS20");
                $this->SendDebug("String to AIO Gateway","http://".$aiogatewayip."/command?auth=".$GatewayPassword."&XC_FNC=LearnSC&type=FS20",0);
            }
            else
            {
                $address = file_get_contents("http://".$aiogatewayip."/command?XC_USER=user&XC_PASS=".$GatewayPassword."&XC_FNC=LearnSC&type=FS20");
                $this->SendDebug("String to AIO Gateway","http://".$aiogatewayip."/command?XC_USER=user&XC_PASS=".$GatewayPassword."&XC_FNC=LearnSC&type=FS20",0);
            }
        }
        else
        {
            $address = file_get_contents("http://".$aiogatewayip."/command?&XC_FNC=LearnSC&type=FS20");
            $this->SendDebug("String to AIO Gateway","http://".$aiogatewayip."/command?&XC_FNC=LearnSC&type=FS20",0);
        }
